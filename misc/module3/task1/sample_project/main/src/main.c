...
#define CCAMNOTE_CONTROLLER_TAG "CCAM Notary Controller"
...
static void read_controller_task(void *pvParameter)
{
  ...
  while (true)
  {
    ...
    ESP_LOGD(CCAMNOTE_CONTROLLER_TAG, "wait for button events");
    if (xQueueReceive(controller_buttons_events, &ev, 50 / portTICK_PERIOD_MS))
    {
      ESP_LOGI(CCAMNOTE_CONTROLLER_TAG, "joystick_buttons_events %d", ev.state);
      buttons = ev.state;
    }
    ...
    ESP_LOGD(CCAMNOTE_CONTROLLER_TAG, "last_js_x %0.2f last_js_y %0.2f", last_js_x, last_js_y);
    ...
    {
      // ESP_LOGI(CCAMNOTE_CONTROLLER_TAG, "send buttons %d JS X=%0.2f Y=%0.2f", buttons, js_x, js_y);
      char message[128];
...
void app_main()
{
  ...
  {
    ESP_LOGE(CCAMNOTE_CONTROLLER_TAG, "%s init read controller failed\n", __func__);
