...
const NobotWorld: React.FC<NobotWorldProps & CCAMOptions> = function NobotWorld({
  world,
  render_frames,
  rotated
}) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (worldRef.current) worldRef.current.destroy();
    }
  }, [world]);
...
export default React.memo(NobotWorld);
