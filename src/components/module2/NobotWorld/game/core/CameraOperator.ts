...
export class CameraOperator implements IInputReceiver, IUpdatable {
   ...
   * The CameraOperator follows the character from its position, allowing for a minor
  ...
  /**
   * Zooms in and out the camera (its target radius)
   * @param delta
   */
  public zoom(delta: number): void {
    this.targetRadius += delta * this.scrollSensitivity;
  ...
  public update(): void {
    ...
    if (this.followMode === true) {
      ...
      if (this.overrideTarget) {
        this.camera.lookAt(this.overrideTarget);
      } else {
        this.camera.lookAt(this.target);
      }
    }
   ...
   * camera from the character, but not so much that this would make clicking on
