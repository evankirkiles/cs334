...
import { AISpawnPoint } from "./AISpawnPoint";
    ...
    root.traverse((child) => {
      if (
        ...
        if (child.userData.data === "spawn") {
          ...
          } else if (child.userData.type === "ai") {
            if (
              Object.prototype.hasOwnProperty.call(child.userData, "character")
            ) {
              const sp = new AISpawnPoint(child, child.userData.character);
              this.spawnPoints.push(sp);
            }
          }
        }
